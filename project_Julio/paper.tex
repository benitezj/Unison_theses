...
In order to evaluate the total number of events we use integrated luminosity, it is a measurement of the collected data size, and it is important value to characterize the performance of an accelerator, defined as: \\ 


\begin{center}
$L_{int} = \int _{0}^{t} L(t') dt'$ \\ 
\end{center}


Because it directly relates to the number of observed events; $L_{int} \sigma$ = number of events of interest. \\
...
The main objective of this work is to obtain the calibration constant for the PCC luminometer with 2024 data aiming for a uncertainty less than 2\% in this Run. This results are important for CMS to compare physical process with the Standard Model, these results are likely to be presented on conferences and reviews on 2025.
...
Based on the previous work on the Run 2 (2015-2016) datasets which have achieved a uncertainty of  2.3\% \cite{Collaboration:121}, the current systematic uncertainty on the Run 2 2017 was 2.3\% \cite{CMS:2018elu} for 2018 was 2.5\% \cite{CMS:2018} and for Run 3 (2022) was 1.4\% \cite{Collaboration:124}. We expect that a precision at 2\% will be possible due to the improved methods we are developing.
...
These scans are performed by varying the separation between the beams in each direction ($x$ and $y$) at a fixed number of separation steps of about 100 micrometers. Then these rates are fitted by a Gaussian model to obtain the beam overlaps $\Sigma_{x,y}$ and the peak rate of clusters $R(0,0)$ \cite{CMS:2018}. A schematic view of this method is shown in Fig. \ref{vdMSketch}. 
...
\begin{equation}
  ...
  \label{sigmavis_eq}
\end{equation}
Where $f$  is the LHC frecuency and $N_{1},N_{2}$ are the number of protons in each colliding bunch, determined from the beam currents.
...
\item Preliminary measurements of the 2024 integrated luminosity for CMS
...
\section{Calendar of activites}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.7\columnwidth]{./diag.jpeg}
  \caption{
...
\item Learn how to use High Performance Computer, Latex and Root
\item Data analysis and Statistical analysis
\item Publication of results
\end{itemize}


\section{Mobility actions}
We expect to participate in a national conference on a congress the next year 2025. 
