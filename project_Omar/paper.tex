...
El proyecto busca realizar análisis y calibración de los datos de luminosidad correspondiente a las mediciones hechas por el experimento CMS para la recta final de la Run 3, durante 2025. Se utiliza para la medición el método PCC (por sus siglas en inglés, Pixel Cluster Counting) que consiste en el conteo de grupos de píxeles activados en el detector. Esto nos permitirá medir con precisión el valor de luminosidad para los datos recabados durante este año. La luminosidad y su incertidumbre sistemática son parámetros de suma importancia para las mediciones de física realizadas en los experimentos en el LHC.
...
For the second part of Run 2 (2017-2018), the integrated luminosity is about \lumi{78}, but its precise value and uncertainty remain to be determined \cite{CMS:2018elu-corr}.
...
%The main porpuse is to determine the measurement corresponding to 2025 data
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\columnwidth]{./LHCPlan.png}
  \caption{CERN's current plan for reaching the High Luminosity LHC. The Long Shutdown 3 (LS3) is now scheduled starting July 2026, seven and a half months later than planned, and with a duration increased by four months. The start of Run 4 is now programmed for 2030 \cite{Lamont:2024}.}
  \label{figure1}
\end{figure}
...
The detector uses a powerful magnet to bend charged particles as they leave the point of collision, allowing to analyze its curvature to identify charge and measure their momentum. A silicon tracker, consisting of about 75 million sensors arranged in concentric layers, is in charge of measuring said curvature with high precision. It has the form of multiple cylindrical layers, and a large number of components. The electromagnetic calorimeter detects photons and electrons while the hadron calorimeter detects mostly pions ($\pi$) and kaons ($K$). Muons ($\mu$) are detected by special chambers outside the solenoid, as shown in . 

%insertar imagen del detector
...
A pixel cluster is for a group of contiguous pixels activated by a charged particle. The method of choice, PCC, exploits the huge number of pixel in the interior part of the CMS pixel detector, which means that the probability that a single pixel is hit by two charged particles from the same bunch crossing is extremely small.%Be more specific about the interior part

To ensure accurate and stable luminosity measurement, there is a detector module selection process, where a veto list is made each period and then combined for the entire year. This process involves determining the reliability of each module based on analyzing what is referred as the module weight, which is the change in the ratio of module PCC and total PCC. The veto list consists of modules excluded due to their known and suspected issues that affect the accuracy of the measurement, such as radiation damage, material defects, contamination, along other issues. At the end of said process, around 10\% of all modules are used. 

The LHC has two main types of runs: physics runs (which could be proton-proton, heavy-ion, or proton-lead collisions,) and callibration runs. As the title of the project suggests, callibration runs are the matter at hand, and consist of more controlled collisions to be able to measure luminosity and other parameters. In these runs, it is necessary to take into consideration a background noise contribution to the PCC rates. Each type of contribution is estimated independently during Super Separation (SS) periods, that are performed to separate two beams a certain distance for a selected period of time. To estimate the background value and its statistical error, the mean value and standard deviation are taken from the distribution resulting of a Y-projection of an average of pixel clusters per event ($\langle PCC \rangle$). Once the noise is taken into account, the data provided can be saved.

To get the van der Meer (vdM) scan data, the pixel detector is read with a trigger of aproximately 50 kHz for about 10 BCIDs of the 140 that collide. The data is then saved distributed in 32 different datasets of events. 

%----------------------------------------------------------------------------------------------------------------------------------------------------

To calibrate the luminometer, the vdM scan technique is employed in a special LHC run, usually at the beginning of the year or period. To perform these scans, the separation between proton beams is varied in two dimensions, in separation steps of 100 micrometers. It measures the rates of clusters using the PCC algorithm and the beam parameters during vdM, to determine the calibration constant ($\sigma_{vis}$).

The definition of instantaneous luminosity for a single colliding bunch pair is
...
The parameter $\sigma_{vis}$ is known as the calibration constant, and is used to normalize the measured PCC rates so the luminosity can be determined during normal running throughout the data-taking year.%reFERENCES!!!!!!!!!!!
...
To warrant stable luminosity measurement for Run 3 2025, the set goals are:

\begin{itemize}
  %\item Process all the datasets. %% not a specific objective, a que se refiere %%
  \item Apply a selection of good modules from the detector: To assure an accurate measurement, a veto list is made, taking only modules with relatively consistent contributions, and then used to only process zero bias PCC data for the vdM analysis.%Check module selection part
  \item Measure the noise in the modules: A calculation of the background contributions, made during selected time windows, to account for secondary particles, and other unwanted parts. % Estimation over separation periods
  %\item Select the time windows corresponding to the vdM scans. % Mentira, no entendi
  \item Set the ideal model adjustment: The generated rates of the scan need to be fitted a mathematical function to extract the parameters needed. %% igual, no es especifico, basicamente checar ajuste segun el ruido %%
  \item Measure the beam parameters $\Sigma_i$, conducting scans in the $x$ and $y$ directions, determined through curved fitting of the scan data. 
  \item Measure the calibration constant $\sigma_{vis}$ to normalize the PCC rates. 
  \item Determine the systematic uncertainties on the integrated luminosity: It is necessary to recognize the effects on the beam overlap measurement, and therefore the calculation of $\sigma_{vis}$, so corrections are to be applied.
\end{itemize}
...
\begin{itemize}
  ...
  \item Calculate the parts of the systematic uncertainty corresponding the work group.
\end{itemize}
