\chapter{Luminosity measurement at the CMS experiment}  %Title of the First Chapter
...
\[
\lambda = -\ln(P(0; \lambda))
\]

The average number of events per bunch crossing, $\lambda$, is related to the instantaneous luminosity, $L_{inst}$, and the total visible cross-section, $\sigma_{\text{vis}}$, by:

\[
\lambda = L_{inst} \cdot \sigma_{\text{vis}}
\]

Substituting $\lambda$ in terms of $L$ and $\sigma_{\text{vis}}$ into the equation involving $P(0; \lambda)$, we get:

\[
L_{inst} \cdot \sigma_{\text{vis}} = -\ln(P(0; \lambda))
\]

Solving for $L_{inst}$ gives us the equation for instantaneous luminosity based on the zero-counting method:

\[
L_{inst} = \frac{-\ln(P(0; \lambda))}{\sigma_{\text{vis}}}
\]

The probability $P(0; \lambda)$ can be written as the ratio $\frac{N}{N_0}$ because it is the fraction of the total number of events where zero particles are detected:
\[
P(0; \lambda) = \frac{N}{N_0}
\]

Substituting $P(0; \lambda) = \frac{N}{N_0}$ into the equation for $L_{inst}$ gives us:

\begin{equation}
L_{inst} = \frac{-ln(N/N_0)}{\sigma_{vis}}
\end{equation}

where $L_{inst}$ is the luminosity, N is the number of events with zero particles detected in the region of interest, $N_0$ is the total number of events, $\sigma_{vis}$ is the visible cross-section. One advantage is that it is a simple and robust method that does not require sophisticated detectors or calibration procedures. However, it can be affected by background events, such as cosmic rays or beam-gas interactions. \\
The HFET algorithm measures luminosity by summing transverse energy deposits in the HF calorimeter cells. It relies on the direct proportionality between the total transverse energy and the particle flux from the interaction point. Accurate calibration of calorimeter cells is crucial for this method, as it ensures that the aggregate transverse energy accurately reflects the luminosity of the particle collisions \cite{Sirunyan:2759951}.

%The Hadron Forward Transverse Energy (HFET) algorithm is a method used to determine the luminosity of particle collisions. This method is based on the direct proportionality between the transverse energy deposited in the Hadron Forward calorimeter cells and the particle flux from the collision point.

The measurement of instantaneous luminosity, $L_{inst}$, using the HFET algorithm involves the following steps:

\begin{enumerate}
  \item Initialize the total transverse energy sum: 
  \[ \Sigma E_T = 0 \]
  
  \item For each calorimeter cell, do:
  \begin{enumerate}
    \item Measure the transverse energy \( E_{T,\text{cell}} \).
    \item Update the total transverse energy sum: 
    \[ \Sigma E_T = \Sigma E_T + E_{T,\text{cell}} \]
  \end{enumerate}
  
  \item Correct the total transverse energy sum for background energy:
  \[ \Sigma E_T = \Sigma E_T - E_{T,\text{background}} \]
  
  \item Multiply by a calibration constant \( k \) to obtain the luminosity:
  \[ L_{inst} = \( k \ \times \Sigma E_T \]
\end{enumerate}

The calibration constant \( k \) is empirically determined through a series of calibration procedures, van der Meer scans. The constant accounts for the proportionality between the aggregate transverse energy and the actual luminosity of the particle collisions.
...
%The DT system in the CMS detector is used for muon detection and precise position measurement. Drift Tubes are essentially gas-filled detectors. When muons pass through these tubes, they ionize the gas. The resulting ions and electrons drift under the influence of an electric field to anode wires located in the center of the tubes, producing a signal. By measuring the drift time, one can infer the position of the muon's trajectory.
  %To use the DT muon rates as a luminometer, one would need to count the rate of detected muons as a function of the delivered luminosity.
...
\item The number of pixel clusters in each event is affected by various sources of background noise. %such as electronic noise and cosmic ray background.
